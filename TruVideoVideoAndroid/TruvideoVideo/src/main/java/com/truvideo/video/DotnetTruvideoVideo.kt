...
class DotnetTruvideoVideo {
    companion object {
        ...
        fun getVideoInfo(context: Context, inputPath: String,callback: VideoCallback) {
            CoroutineScope(Dispatchers.Main).launch {
                try {
                    ...
                    //Log.d("IMPORTANT_TAG", "duration :${duration}")
                    //Log.d("IMPORTANT_TAG", "width :${width}")
                    val gson = Gson()
        ...
        @JvmStatic
        fun generateThumbnail(context: Context, inputPath: String, outputPath: String,callback: VideoCallback) {
            var input_Path = videoFile(inputPath)
            ...
            CoroutineScope(Dispatchers.Main).launch {
                try {
                    ...
                    //Log.d("IMPORTANT_TAG", "result :${resultPath}")
        ...
        @JvmStatic
        fun clearNoise(context: Context, inputPath: String, outputPath: String,callback: VideoCallback) {
            var input_Path = videoFile(inputPath)
            var output_Path = videoFileDescriptor(outputPath)
            CoroutineScope(Dispatchers.Main).launch {
                try {
        ...
        @JvmStatic
        fun mergeVideos(context: Context, arrayList: List<String>, outputPath: String,callback: VideoCallback) {
        ...
        @JvmStatic
        fun concatVideos(context: Context, arrayList: List<String>, outputPath: String,callback: VideoCallback) {
            var input_Path = listVideoFile(arrayList)
            var output_Path = videoFileDescriptor(outputPath)
            CoroutineScope(Dispatchers.Main).launch {
                try {
        ...
        @JvmStatic
        fun encodeVideo(context: Context, inputPath: String, outputPath: String,callback: VideoCallback) {
            var input_Path = videoFile(inputPath)
            var output_Path = videoFileDescriptor(outputPath)
            CoroutineScope(Dispatchers.Main).launch {
                try {
